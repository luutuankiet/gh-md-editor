<script lang="ts">
  ...
  function sampleDoc(): string {
    return [
      ...
      '3. Press `Cmd/Ctrl+Shift+→` repeatedly → selection grows: word → fence → MARKDOWN SECTION → ENCLOSING PARENT SECTION → document. Mac: `Ctrl+Shift+→` also bound.',
      '4. Select a word and press `Cmd/Ctrl+D` repeatedly → spawn cursors at every next match. Type once, type everywhere.',
      '5. Hold `Alt/Opt` and left-click anywhere → drop another cursor at the click point.',
      '6. Click the trash icon in the outline header (top-right) → wipes localStorage and reloads, restoring this tour.',
      '',
      ...
      '| Scrollbar match ticks | 18px gutter, yellow search matches, orange active | same shades, same gutter |',
